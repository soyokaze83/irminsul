...
mod tests {
    ...
    fn conformance_hex(s: &str) -> Vec<u8> {
        (0..s.len())
            .step_by(2)
    ...
    async fn signal_conformance_decrypts_libsignal_pre_key_message() {
        ...
        .unwrap();
        let bob = &fx["bob"];
        let kp = |pubk: &[u8], privk: &[u8]| KeyPair {
            public: <[u8; 32]>::try_from(&pubk[pubk.len() - 32..]).unwrap(),
            private: SecretBytes::from(privk.to_vec()),
        };
        let id_pub = conformance_hex(bob["identityKeyPub"].as_str().unwrap());
        let id_priv = conformance_hex(bob["identityKeyPriv"].as_str().unwrap());
        let spk_pub = conformance_hex(bob["signedPreKeyPub"].as_str().unwrap());
        let spk_priv = conformance_hex(bob["signedPreKeyPriv"].as_str().unwrap());
        let spk_sig = conformance_hex(bob["signedPreKeySig"].as_str().unwrap());
        let pk_pub = conformance_hex(bob["preKeyPub"].as_str().unwrap());
        let pk_priv = conformance_hex(bob["preKeyPriv"].as_str().unwrap());
        let pk_id = bob["preKeyId"].as_u64().unwrap() as u32;

        let store = temp_store().await;
        let mut creds = create_initial_credentials().unwrap();
        creds.registration_id = bob["registrationId"].as_u64().unwrap() as u32;
        creds.signed_identity_key = kp(&id_pub, &id_priv);
        creds.signed_pre_key = crate::SignedPreKey {
            key_id: bob["signedPreKeyId"].as_u64().unwrap() as u32,
            key_pair: kp(&spk_pub, &spk_priv),
            signature: Bytes::from(spk_sig),
        };
        save_credentials(&store, creds).await.unwrap();
        let otk = kp(&pk_pub, &pk_priv);
        let mut enc = [0u8; 64];
        enc[..32].copy_from_slice(&otk.public);
        enc[32..].copy_from_slice(otk.private.expose());
        store
            .set_signal_key(KeyNamespace::PreKey, &pk_id.to_string(), &enc)
            .await
            .unwrap();

        let v0 = &fx["vectors"].as_array().unwrap()[0];
        let ct = conformance_hex(v0["body"].as_str().unwrap());
        let codec = SignalMessageCodec::new(
            StoreSignalRepository::new(store.clone()),
            StoreSignalSenderKeyProvider::new(store.clone()),
        );
        let plaintext = codec
            .decrypt_inbound_message(InboundEncryptedPayload {
